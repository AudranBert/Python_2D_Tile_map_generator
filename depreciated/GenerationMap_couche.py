...
def caseadjacente(map, i,
                  j):  # i=longueur et j=hauteur terre sous forme de tube donc possibilite de passer d un coté a l autre
    caseadj = []
    if (j > 0):
...
#### export html
BiomeColors = ["#000000", "#3c4adb", "#67c1fa", "#443912", "#54d239", "#ba762d", "#afafaf", "#ffffff", "#D5BF0D",
               "#0066CC", "#336600"]
...
def export_html(L, nomhtml):
    ...
    with open(nomhtml, 'w') as fichier:
        ...
        for i in range(0, hauteur):
            ...
            fichier.write("			<tr>\n")
            for j in range(0, longueur):
                fichier.write('<td class="d' + str(map[i][j]) + '">' + "<pre class=R" + str(mapr[i][j]) + " > " + str(
                    mapr[i][j]) + " </pre>" + "</td>\n")
            fichier.write("			</tr>\n")
            modul = ligne % 50
            if modul == 0:
...
def export_img(L, nomhtml):
    export_time = time.time()
    # export image
    from PIL import Image, ImageDraw
    img = Image.new('RGB', (longueur, hauteur), color=(0, 0, 0))
    draw = ImageDraw.Draw(img)
    for i in range(0, hauteur):
        for j in range(0, longueur):
            draw.point((j, i), fill=BiomeColors[map[i][j]])
    img.save(nomhtml[:-5] + ".png")
    print("Duree l'export HTML : %s secondes ---" % (time.time() - export_time))
